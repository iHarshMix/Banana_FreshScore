...
with tab1:
    ...
    st.write(
        "Upload your own photo or **click any sample banana below** to instantly "
        "estimate continuous ripeness score, remaining shelf life, and retail actions."
    )
    ...
    # Initialize default image on first load
    if "current_image" not in st.session_state:
        default_p = example_samples["🟡 Fresh Ripe"]
        if default_p.exists():
            st.session_state["current_image"] = Image.open(default_p).convert("RGB")
            st.session_state["image_source"] = "🟡 Fresh Ripe (Sample Example)"

    with col1:
        st.markdown("### 📤 Image Input & Examples")

        # Gradio-style Examples row
        st.markdown("**✨ Click an example to test instantly (Gradio-style):**")
        ex_cols = st.columns(4)
        for idx, (lbl, p) in enumerate(example_samples.items()):
            with ex_cols[idx]:
                if p.exists():
                    st.image(Image.open(p), use_column_width=True)
                    if st.button(lbl, key=f"ex_btn_{idx}", use_container_width=True):
                        st.session_state["current_image"] = Image.open(p).convert("RGB")
                        st.session_state["image_source"] = f"{lbl} (Sample Example)"

        st.markdown("---")
        uploaded_file = st.file_uploader(
            "Or Upload Your Custom Image (JPG, PNG, WEBP)",
            type=["jpg", "jpeg", "png", "webp"],
            help="Drop any banana photo here to analyze",
        )
        if uploaded_file is not None:
            st.session_state["current_image"] = Image.open(uploaded_file).convert("RGB")
            st.session_state["image_source"] = f"Custom Upload: {uploaded_file.name}"

        # Real-time Live Sensor Drift Simulator
        with st.expander("🌪️ Simulate Warehouse Camera Sensor Shift Live"):
            ...
            if st.button("Apply Camera Drift to Current Image", use_container_width=True):
                if "current_image" in st.session_state and st.session_state["current_image"]:
                    ...
                    st.session_state["current_image"] = apply_synthetic_drift(
                        st.session_state["current_image"],
                        hue_shift_deg=hue_slider,
                        brightness_factor=bright_slider,
                        blur_radius=blur_slider,
                    )
                    st.session_state["image_source"] = "Simulated Camera Drift"

        selected_image = st.session_state.get("current_image")
        source_label = st.session_state.get("image_source", "Active Image")

        if selected_image is not None:
            st.image(
                selected_image,
                caption=f"Active Input: {source_label}",
